    ...
    for(let i=0; i<monthArray.length; i++){
        if(monthArray[i] == currentDay){
            ...
            break;
                ...
                monthArray[i] == currentDay+1 ||
                i == monthArray.length-1 && dayA == 1 && (
                    ((currentDay == 28 || currentDay == 29) && currentMonth == february) ||
                    (currentDay == 30 && thirtyDaysMonths.indexOf(currentMonth) > -1) ||
                    (currentDay == 31 && thirtyOneDaysMonths.indexOf(currentMonth) > -1)
                )
            ){
...
function isTomorrowB(currentDay, dayA, dayB, language){
    ...
    if(
        ((dayA == 28 || dayA == 29) && currentMonth == february) || 
        (dayA == 30 && thirtyDaysMonths.indexOf(currentMonth) > -1) || 
        (dayA == 31 && thirtyOneDaysMonths.indexOf(currentMonth) > -1) && dayB == 1
    ) {
...
function revealDisplayC(currentMonth, currentDay){
    ...
    const march = 2;
    const october = 9;
    const arrayCarnaval = [9,10,11,12,13];
    ...
    if((currentMonth == february && arrayCarnaval.includes(currentDay)) || (currentMonth == march && arrayFallas.includes(currentDay)) || (currentMonth == october && arrayInktober.includes(currentDay))){
        return true;
    }
    else{
        return false;
...
function getMonthName(currentMonth, language){
    ...
    switch(currentMonth){
        case 0:
            return checkLanguage(language, "enero", "January");
            break;
        case 1:
            return checkLanguage(language, "febrero", "February");
            break;
        case 2:
            return checkLanguage(language, "marzo", "March");
            break;
        case 3:
            return checkLanguage(language, "abril", "April");
            break;
        case 4:
            return checkLanguage(language, "mayo", "May");
            break;
        case 5:
            return checkLanguage(language, "junio", "June");
            break;
        case 6:
            return checkLanguage(language, "julio", "July");
            break;
        case 7:
            return checkLanguage(language, "agosto", "August");
            break;
        case 8:
            return checkLanguage(language, "septiembre", "September");
            break;
        case 9:
            return checkLanguage(language, "octubre", "October");
            break;
        case 10:
            return checkLanguage(language, "noviembre", "November");
            break;
        case 11:
            return checkLanguage(language, "diciembre", "December");
            break;
        default:
            break;
    }
